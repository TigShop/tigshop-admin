<template>
    ...
    <MyModal v-if="loaded" :class="dialogClass" v-model:open="visible" :confirm-loading="confirmLoading" :width="_width" :title="_title" okText="确认" cancelText="取消"
        :destroyOnClose="true" @ok="onOk" :ok-button-props="{ disabled: okDisabled }" :footer-style="{ textAlign: 'right' }" v-bind="bindParams" :bodyStyle="bodyStyle">
        <a-config-provider :locale="zhCN">

            <Component ref="submitForm" @submitCallback="submitCallback" @callback="callback" @show="show" @close="close" @okType="okType" v-bind="params"
                v-model:confirmLoading="confirmLoading">
            </Component>
        </a-config-provider>
        <template v-if="showClose || showOnOk" #footer>
            <a-button v-show="showClose" style="margin-right: 12px" @click="close">取消</a-button>
            <a-button v-show="showOnOk" :disabled="okDisabled" :loading="confirmLoading" type="primary" @click="onOk">确认</a-button>
        </template>
    </MyModal>
</template>
