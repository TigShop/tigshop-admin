<template>
    <div class="lyecs-article-select-group">
        <el-space>
            <DialogForm :params="{ selected_ids: modelValue, isMultiple: props.isMultiple }" isDrawer path="content/article/src/SelectArticle" title="选择文章" width="600px" @okCallback="onOk">
            ...
            <el-button v-if="isMultiple && modelValue.length > 0" @click="clear">清空</el-button>
        </el-space>
...
const loadList = async (ids:any) => {
    loading.value = true;
    try {
        const result = await getArticleList({article_ids: ids, ...filterParams});
        articleList.value = result.filter_result;
