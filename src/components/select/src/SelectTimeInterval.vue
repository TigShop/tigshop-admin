<template>
    <div class="lyecs-time-select-group">
        <div>
            <el-date-picker :clearable="!clearable" v-model="startDate" :type="type" :disabled-date="disabledStartDate"
                placeholder="开始日期" @change="onChangeStartDate" :value-format="valueFormat" :editable="false"></el-date-picker>
            <span style="display: inline-block;padding: 0 10px;">至</span>
            <el-date-picker :clearable="!clearable" v-model="endDate" :type="type" :disabled-date="disabledEndDate"
                placeholder="结束日期" @change="onChangeEndDate" :value-format="valueFormat" :editable="false"></el-date-picker>
        </div>
    </div>
</template>
