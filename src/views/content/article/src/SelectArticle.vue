                    ...
                    <a-spin :spinning="loading">
                        <div style="padding-right: 15px; box-sizing: border-box">
                            <el-table
                                ...
                                <el-table-column label="文章名称" prop="article_title"></el-table-column>
                                <template #empty>
...
// 基本参数定义
const columns: TableColumnsType = [{ title: "文章", key: "article_id", sorter: true }];
...
const selectRow = (selection: ArticleFilterState[], val: ArticleFilterState) => {
    if (!props.isMultiple) {
        ...
        selectedIds.value.push(val.article_id);
        emit("okType", selectedIds.value.length > 0);
    }
    linkSelectData.value = {
        ...
        label: "文章",
        id: val.article_id,
        name: val.article_title,
    };
};
const isSelectable = (row: ArticleFilterState) => {
    // 排除重复项
    return !props.selected_ids.includes(row.article_id); // Column configuration not to be checked
};
// 弹窗回调
const onFormSubmit = () => {
    // 弹窗确认按钮提交
