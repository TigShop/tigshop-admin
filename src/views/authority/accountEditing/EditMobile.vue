        ...
        <div class="content_wrapper">
            <div class="lyecs-form-table">
                <el-form ref="formRef" :model="formState" label-width="auto">
                    <el-form-item label="手机号：" prop="mobile" :rules="[{ required: true, validator: validateMobile, trigger: 'blur' }]">
...
import { adminInfoSubmit, getAdminCode } from "@/api/authority/accountEditing";
...
const onCode = async () => {
    ...
    try {
        const result = await getAdminCode({ type: 'code', ...formState.value });
...
const onSubmit = async () => {
    try {
        ...
        const result = await adminInfoSubmit({modify_type: 3, ...formState.value});
