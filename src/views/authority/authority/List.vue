            ...
            <div class="lyecs-table-list-warp">
                        ...
                        <div class="list-table-tool-row">
                            <div class="list-table-tool-col">
                                <el-space>
                                    ...
                                    <DialogForm :params="{ act: 'add', parentId: filterParams.parent_id }" isDrawer path="authority/authority/Info" title="添加权限" width="600px"
                ...
                <div class="table-container">
                    <a-spin :spinning="loading">
                        <el-table :data="filterState" :expand-row-keys="expandIds" :indent="30" :load="loadData" :total="total" :tree-props="{ children: 'children', hasChildren: 'has_children' }" lazy
                                  row-key="authority_id" @selection-change="onSelectChange" @expand-change="handleExpandChange" ref="tableRef">
                            <el-table-column type="selection" width="32"/>
                            <el-table-column :width="260" label="权限名称">
                                <template #default="{ row }" row-key="authority_id">
                                    <el-space style="height: 30px;">
                                        <div style="position: relative;">
                                            <PopForm v-model:org-value="row.authority_name" :params="{ id: row.authority_id, field: 'authority_name' }" :requestApi="updateAuthorityFiled" label="权限名称"
                                                     type="textarea">
                                                <a v-if="row.has_children" @click="childCat(row.authority_id)">{{ row.authority_name }}</a>
                                                <span v-else>{{ row.authority_name }}</span>
                                            </PopForm>
                                        </div>
                                    </el-space>
                                </template>
                            ...
                            <el-table-column :width="100" label="权限图标">
                                <template #default="{ row }">
                                    <em :class="row.authority_ico" class="main_pel_m"></em>
                                </template>
                            </el-table-column>
                            <el-table-column label="唯一编码">
                                <template #default="{ row }">
                                    <PopForm v-model:org-value="row.authority_sn" :params="{ id: row.authority_id, field: 'authority_sn' }" :requestApi="updateAuthorityFiled" label="唯一编码"
                                             type="textarea">
                                        <div>{{ row.authority_sn || '-' }}</div>
                                    </PopForm>
                                </template>
                            </el-table-column>
                            <el-table-column label="路由地址">
                                <template #default="{ row }">
                                    <PopForm v-model:org-value="row.route_link" :params="{ id: row.authority_id, field: 'route_link' }" :requestApi="updateAuthorityFiled" label="路由地址"
                                             type="textarea">
                                        <div>{{ row.route_link || '-' }}</div>
                                    </PopForm>
                                </template>
                            </el-table-column>
                            <el-table-column label="细分权限">
                                <template #default="{ row }">
                                    <span v-for="(child, idx) in row.child_auth">{{ idx != 0 ? ',' : '' }}{{ child.auth_name }}</span>
                                </template>
                            </el-table-column>
                            <el-table-column key="is_show" :sorter="true" label="是否显示">
                                <template #default="{ row }">
                                    <Switch v-model:checked="row.is_show" :params="{ id: row.authority_id, field: 'is_show' }" :requestApi="updateAuthorityFiled"/>
                                </template>
                            </el-table-column>
                            <el-table-column key="sort_order" :sorter="true" label="排序">
                                <template #default="{ row }">
                                    <PopForm v-model:org-value="row.sort_order" :params="{ id: row.authority_id, field: 'sort_order' }" :requestApi="updateAuthorityFiled" label="排序"
                                             type="input">
                            ...
                            <el-table-column :width="220" fixed="right" label="操作">
                                <template #default="{ row }">
                                    ...
                                    <DialogForm :params="{ act: 'add', parentId: row.authority_id }" :data="{parentId: row.parent_id || row.authority_id}" isDrawer path="authority/authority/Info" title="编辑权限" width="600px"
                                                @okCallback="updateChildFilter">
                                        <a class="btn-link">添加子目录</a>
                                    </DialogForm>
                                    <el-divider direction="vertical"/>
                                    <DeleteRecord v-if="row.parent_id" :params="{ id: row.authority_id }" :requestApi="delAuthority" @afterDelete="updateChildFilter({}, {parentId: row.parent_id})">删除</DeleteRecord>
                                    <DeleteRecord v-if="!row.parent_id" :params="{ id: row.authority_id }" :requestApi="delAuthority" @afterDelete="loadFilter">删除</DeleteRecord>
                                </template>
                            </el-table-column>
                            <template #empty>
...
import type {FriendLinksFilterState} from "@/types/setting/friendLinks";
//获取来自路由的参数
...
const loadFilter = async () => {
    ...
    try {
        ...
        if (!result.parent_name) {
            ...
            appStore.setHeaderTitle('权限列表 - ' + result.parent_name)
        }
    } catch (error:any) {
...
const loadData = async (tree:any, treeNode:any, resolve:any) => {
	...
	// currentParentId.value = tree.authority_id
    // 在之前声明的全局变量中，增加一个key为 本条数据的id，id可替换为你数据中的任意唯一值
    tableTreeRefreshTool[tree.authority_id] = {}
    // 重要！保存resolve方法，以便后续使用
    tableTreeRefreshTool[tree.authority_id].resolve = resolve
    // 记录展开次数，具体作用后续介绍
    tableTreeRefreshTool[tree.authority_id].expandCount = 0
    // 请求api接口获取数据
    const result = await getAuthorityList({
        page: 1,
        parent_id: tree.authority_id,
        size: 1000,
    });
    // 调用resolve方法，渲染子级数据
...
const handleExpandChange = async (row:any, expanded:any) => {
    // 获取到之前保存的全局变量
    const curr = tableTreeRefreshTool[row.authority_id]
    // 展开次数 +1
    curr.expandCount++
...
const onSelectChange = (e: AuthorityFilterState[]) => {
    selectedIds.value = e.map((item) => item.authority_id);
}
</script>
<style lang="less">
