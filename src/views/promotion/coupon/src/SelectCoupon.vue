                    ...
                    <el-form :model="filterParams" name="form" @finish="searchSubmit">
                        <div class="list-table-tool-row" style="margin-bottom: 5px">
                            <div class="list-table-tool-col">
                                <el-space>
                                    <el-input v-model="filterParams.keyword" placeholder="请输入搜索优惠券" />
                    ...
                    <a-spin :spinning="loading">
                        <div style="padding-right: 15px; box-sizing: border-box">
                            ...
                            >
                                ...
                                <el-table-column label="优惠券名称" prop="coupon_name"></el-table-column>
                                <template #empty>
...
emit("okType", false);
// 基本参数定义
const columns: TableColumnsType = [{ title: "商品信息", key: "coupon_id", sorter: true }];
...
const onSelectChange = (e: any) => {
    if (props.isMultiple) {
        selectedIds.value = e.map((item: CouponFilterState) => item.coupon_id);
...
const selectRow = (selection: any, val: any) => {
    if (!props.isMultiple) {
        ...
        selectedIds.value.push(val.coupon_id);
        emit("okType", selectedIds.value.length > 0);
...
const isSelectable = (row: CouponFilterState) => {
    // 排除重复项
    return !props.selected_ids.includes(row.coupon_id); // Column configuration not to be checked
};
// 弹窗回调
const onFormSubmit = () => {
    // 弹窗确认按钮提交
