                    ...
                    <div class="card-wrap__container">
                        <div class="item-bg coupon-cards-item">
                            ...
                            <DialogForm isDrawer @okCallback="loadFilter" title="添加减免优惠活动" width="800px"
                                path="promotion/productActivity/Info" :params="{ act: 'add', promotion_type: 1 }">
                        ...
                        <div class="item-bg coupon-cards-item">
                            ...
                            <DialogForm isDrawer @okCallback="loadFilter" title="添加折扣优惠活动" width="800px"
                                path="promotion/productActivity/Info" :params="{ act: 'add', promotion_type: 2 }">
                                <el-button type="primary">立即新建</el-button>
                            </DialogForm>
                        </div>
                        <div class="item-bg coupon-cards-item item-bg2">
                            ...
                            <DialogForm isDrawer @okCallback="loadFilter" title="添加赠品优惠活动" width="800px"
                                path="promotion/productActivity/Info" :params="{ act: 'add', promotion_type: 0 }">
                                <el-button type="primary">立即新建</el-button>
                            </DialogForm>
                        </div>
                    </div>
                    ...
                    <el-form :model="filterParams">
                        <div class="list-table-tool-row">
                            <div class="list-table-tool-col">
                                <el-space>
                                    <el-input name="keyword" v-model="filterParams.keyword" placeholder="输入活动名称">
                                        <template #append>
                                    ...
                                    <a :class="'advanced-search-btn ml18 ' + (advancedSearch ? 'active' : '')"
                                        @click="advancedSearch = !advancedSearch">高级搜索<i class="iconfont icon-xia"></i></a>
                                </el-space>
                            </div>
                            <div class="list-table-tool-col">
                            </div>
                ...
                <div class="table-container">
                    <a-spin :spinning="loading">
                        <el-table :data="filterState" row-key="coupon_id" @selection-change="onSelectChange" :total="total"
                            ...
                            <el-table-column label="活动名称" prop="promotion_name" :width="150">
                                <template #default="{ row }">
                                    ...
                                    <p class="order-tips-p">
                                        <span class="green">
                                            {{ row.promotion_type == 1 ? '减免' : row.promotion_type == 2 ? '折扣' : '赠品'}}
                                        </span>
                                    </p>
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column label="金额下限" prop="min_order_amount" sortable="custom">
                                <template #default="{ row }">
                                    <!-- <PopForm label="金额下限" type="input" :requestApi="updateProductActivityFiled" v-model:org-value="row.min_order_amount"
                                        :params="{ id: row.promotion_id, field: 'min_order_amount' }"> -->
                                        <div>{{ row.min_order_amount }}</div>
                                    <!-- </PopForm> -->
                                </template>
                            </el-table-column>
                            <el-table-column label="金额上限" prop="max_order_amount" sortable="custom">
                                <template #default="{ row }">
                                    <!-- <PopForm label="金额上限" type="input" :requestApi="updateProductActivityFiled" v-model:org-value="row.max_order_amount"
                                        :params="{ id: row.promotion_id, field: 'max_order_amount' }"> -->
                                        <div>{{ row.max_order_amount }}</div>
                                    <!-- </PopForm> -->
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column label="是否启用" prop="is_show" sortable="custom">
                                <template #default="{ row }">
                            ...
                            <el-table-column label="操作" fixed="right">
                                <template #default="{ row }">
                                    <DialogForm isDrawer @okCallback="loadFilter" title="编辑优惠活动" width="800px"
                                        path="promotion/productActivity/Info" :params="{ act: 'detail', id: row.promotion_id }">
...
import { getProductActivityList, batchSubmit, updateProductActivityFiled, delProductActivity } from "@/api/promotion/productActivity";
...
const onSelectChange = (e: ProductActivityFilterState[]) => {
    selectedIds.value = e.map((item) => item.promotion_id);
};
</script>
<style>
/*优惠券列表类型选择*/
