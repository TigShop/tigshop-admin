        ...
        <div class="content_wrapper">
            <div class="lyecs-form-table">
                <el-form v-if="!loading" ref="formRef" :model="formState" label-width="auto">
                    <el-form-item :rules="[{ required: true, message: '标题不能为空!' }]" label="标题" prop="brand_name">
                        <el-input v-model="formState.brand_name"/>
                    </el-form-item>
                    <el-form-item label="内容" :rules="[{ required: true, message: '内容不能为空!' }]" prop="brand_desc">
                        <el-input v-model="formState.brand_desc" row="2" type="textarea"/>
                    </el-form-item>
                    <el-form-item label="发送类型" prop="brand_is_hot">
                        <el-select v-model="formState.send_user_type">
                            <el-option :value="0" label="全部会员" />
                            <el-option :value="1" label="单个会员" />
                            <el-option :value="2" label="会员等级" />
                            <el-option :value="3" label="部分会员" />
                        </el-select>
                        <template v-if="formState.brand_is_hot===1">

                        </template>
                        <template v-else-if="formState.brand_is_hot===2">
                            <SelectRankList v-model="formState.brand_desc"></SelectRankList>
                        </template>
                        <template v-else-if="formState.brand_is_hot===3">
                            <el-input v-model="formState.brand_desc" row="2" type="textarea"/>
