            ...
            <div class="lyecs-form-table">
                <el-form v-if="!loading" ref="formRef" :model="formState" label-width="auto">
                    <el-table :data="formState.items" style="width: 100%">
                        ...
                        <el-table-column label="商品名称" prop="product_id" width="320">
                            <template #default="{ row }">
                                <ProductCard :pic_thumb="row.pic_thumb" :product_id="row.product_id" :product_name="row.product_name" :product_type="row.product_type" :url="row.url">
                                </ProductCard>
                            </template>
                        </el-table-column>
                        <el-table-column label="货号" prop="product_sn" />
                        <el-table-column label="价格" prop="price">
                            <template #default="{ row }">
                                <el-input v-model="row.price" type="text"></el-input>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column label="属性" prop="product_attr">
                            <template #default="{ row }">
                                <el-input :rows="1" v-model="row.product_attr" type="textarea"></el-input>
                            </template>
                        </el-table-column>
...
const onOk = async (ids: number[]) => {
    try {
        ...
        console.log(result);
        result.product_items.forEach(element => {
            element.quantity = 1;
            element.item_id = 0;
            formState.value.items.push(element)
        });
        console.log(formState.value.items)
    } catch (error) {
    } finally {
        emit('update:confirmLoading', false);
    }
