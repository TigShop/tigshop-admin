...
watchEffect(() => {
    if (filterState && filterState.value?.hasOwnProperty("horizontal_axis")) {
        nextTick(() => {
            myChartOptions.value.xAxis.data = filterState.value.horizontal_axis;
            myChartOptions.value.series[0].data = filterState.value.longitudinal_axis;
            myChart?.setOption(myChartOptions.value);
        });
    }
});
