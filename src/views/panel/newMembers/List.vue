...
const getData = async () => {
    try {
        ...
        const result = await getAddUserTrends({
            ...
            start_end_time: filterParams.start_end_time
        });
        if (result.errcode === 0) {
            // filterState.value = result;

            const { horizontal_axis, longitudinal_axis } = result.filter_result;
            const xAxisData = horizontal_axis.map((item) => {
                let unit = "月";
                if (filterParams.date_type === "2") unit = "日";
                if (filterParams.date_type === "3") unit = "时";
                return item + unit;
            });
            myChartOptions.value.xAxis.data = xAxisData;
            myChartOptions.value.series[0].data = longitudinal_axis;
