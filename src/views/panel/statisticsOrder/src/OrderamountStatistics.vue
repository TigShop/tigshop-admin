...
watchEffect(() => {
    if (filterState && filterState.value?.hasOwnProperty("sales_statistics_data")) {
        nextTick(() => {
            myChartOptions.value.xAxis.data = filterState.value.sales_statistics_data.horizontal_axis;
            myChartOptions.value.series[0].data = filterState.value.sales_statistics_data.longitudinal_axis;
