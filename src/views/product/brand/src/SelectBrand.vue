                ...
                <perfect-scrollbar class="table-container" style="position: absolute;top:50px;bottom:50px;overflow-y: auto;width:100%;">
                    <a-spin :spinning="loading">
                        <div style="padding-right:15px;box-sizing: border-box;">
                            <el-table :class="isMultiple ? '' : 'hide-checkbox'" ref="tableRef" class="result-table" :data="filterState" row-key="brand_id" @select="selectRow"
...
const columns: TableColumnsType = [
    { title: '商品信息', key: 'brand_id', sorter: true },
];
...
const onSelectChange = (e:any) => {
    if (props.isMultiple) {
        selectedIds.value = e.map((item:any) => item.brand_id);
...
const selectRow = (selection:any, val:any) => {
    if (!props.isMultiple) {
        ...
        (selectedIds.value as any).push(val.brand_id)
        emit("okType", (selectedIds.value.length > 0));
...
const isSelectable = (row:any, index:number) => {
    // 排除重复项
    return !props.selected_ids.includes(row.brand_id) // Column configuration not to be checked
}
// 弹窗回调
const onFormSubmit = () => {
    // 弹窗确认按钮提交
