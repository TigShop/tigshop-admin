    ...
    <el-form-item label="属性模板" prop="type">
        <el-space>
            <el-select v-model="attrTplId" placeholder="请选择" style="width: 100%" clearable>
                <el-option v-for="item in <any>props.attrTplList" :label="item.tpl_name" key="item.tpl_id"
                    :value="item.tpl_id" />
            </el-select>
        </el-space>
        <el-button :disabled="!attrTplId" @click="importAttrTpl">确认导入</el-button>
    </el-form-item>
            ...
            <div class="attr-warp" v-for="(item, index) in attrList.normal">
                ...
                <div class="attr-warp-box">
                    <el-space class="attr-row" v-for="(attr, idx) in item.attr_list">
                        ...
                        <div v-if="0" class="iconfont icon-cha1 btn-remove" @click="removeAttr('normal', index, idx)">
                        </div>
                    </el-space>
                </div>
            </div>
                    ...
                    <el-space class="attr-row" v-for="(attr, idx) in item.attr_list" :size="16">
                        ...
                        <el-space class="pic-select-con" :size="0">
                            ...
                            <div class="pic-select">
                                <DialogForm type="gallery" class="" @okCallback="addPic"
                                    :data="{ type: 'spe', index: index, idx: idx }">
                                    <div class="pic-select-img">
                                        <img v-if="attr.attr_pic_thumb" :src="imageFormat(attr.attr_pic_thumb)">
                                    </div>
                                </DialogForm>
                                <i @click="removePic('spe', index, idx)" v-if="attr.attr_pic_thumb"
                                    class="pic-select-del iconfont icon-cha"></i>
                            </div>
                        </el-space>
                        <div v-if="item.attr_list.length > 1" class="iconfont icon-cha1 btn-remove"
                            @click="removeAttr('spe', index, idx)"></div>
                    </el-space>
            ...
            <div class="attr-warp" v-for="(item, index) in attrList.extra">
                ...
                <div class="attr-warp-box">
                    <el-space class="attr-row" v-for="(attr, idx) in item.attr_list" :size="16">
                        ...
                        <div v-if="item.attr_list.length > 1" class="iconfont icon-cha1 btn-remove"
                            @click="removeAttr('extra', index, idx)"></div>
                    </el-space>
                    <el-space class="attr-row">
                    ...
                    <table class="table-product" width="100%" cellspacing="1" cellpadding="3">
                        <tbody>
                            <tr>
                                ...
                                <th>商品条形码</th>
                                <th>一口价 <i class="iconfont tips-hover" data-title="一口价会覆盖除属性价格外的所有价格，慎重设置">&#xe611;</i>
                                </th>
                            </tr>
