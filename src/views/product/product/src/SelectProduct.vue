    ...
    <div class="container" style="position: relative;height: 100%;;">
        <div class="content_wrapper">
            <div class="" style="position: absolute;top:0;bottom:0;overflow-y: auto;width: 100%;">
                ...
                <perfect-scrollbar class="table-container" style="position: absolute;top:50px;bottom:50px;overflow-y: auto;width:100%;">
                    <a-spin :spinning="loading">
                        <div style="padding-right:15px;box-sizing: border-box;">
                            <el-table ref="tableRef" :class="isMultiple ? '' : 'hide-checkbox'" :data="filterState" :loading="loading" :total="total" class="result-table"
                                row-key="product_id" @select="selectRow" @selection-change="onSelectChange">
                                <el-table-column :selectable="isSelectable" type="selection" width="32" />
                                <el-table-column label="商品信息" prop="product_id">
                                    <template #default="{ row }">
                                        <div class="flex">
                                            <div v-if="row.pic_thumb" class="span-pic"><a :href="row.url" target="_blank"><img :src="imageFormat(row.pic_thumb)" height="40"
                                                        width="40"></a>
                                            </div>
                                            <div>
                ...
                <div v-if="total > 0" class="pagination-con" style="position: absolute;bottom: 0;justify-content: right;display:flex;width:100%;margin:0">
                    <Pagination v-model:page="filterParams.page" v-model:size="filterParams.size" :total="total" @callback="loadFilter" layout="slot ,prev, pager, next" :background="false"/>
                </div>
            </div>
        </div>
    </div>
