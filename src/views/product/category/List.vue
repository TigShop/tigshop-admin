                    ...
                    <el-form :model="filterParams" name="form">
                        <div class="list-table-tool-row">
                            <div class="list-table-tool-col">
                                <el-space>
                                    <el-button v-if="filterParams.parent_id > 0" @click="backToParentCat"
                                        ><i class="iconfont icon-fanhui" style="font-size: 14px; padding-right: 5px"></i>
                                        返回上一级
                                    </el-button>
                                    <DialogForm
                                        :params="{ act: 'add', parentId: filterParams.parent_id }"
                                        isDrawer
                    ...
                    <a-spin :spinning="loading">
                        ...
                        >
                                ...
                                <template #default="{ row }" row-key="category_id">
                                    <el-space style="height: 30px">
                                        <div style="position: relative">
                                            <PopForm
                                                v-model:org-value="row.category_name"
                            ...
                            <el-table-column :width="120" label="分类图片">
                                <template #default="{ row }">
                                    <div v-if="row.category_pic">
                                        <el-image :src="imageFormat(row.category_pic)" fit="contain" style="height: 30px; width: 30px" />
                                    </div>
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column label="单位">
                                <template #default="{ row }">
                                    <PopForm
                                        v-model:org-value="row.measure_unit"
                            ...
                            <el-table-column label="是否热门">
                                <template #default="{ row }">
                                    <Switch v-model:checked="row.is_hot" :params="{ id: row.category_id, field: 'is_hot' }" :requestApi="updateCategoryFiled" />
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column :width="240" fixed="right" label="操作">
                                <template #default="{ row }">
                                    <DialogForm
                                        :params="{ act: 'detail', id: row.category_id }"
                                    ...
                                    <DialogForm
                                        :params="{ act: 'add', parentId: row.category_id }"
                                        :data="{ parentId: row.parent_id || row.category_id }"
                                        isDrawer
                                        path="product/category/Info"
                                        title="添加子分类"
                                        width="600px"
                                    ...
                                    <DeleteRecord
                                        ...
                                        @afterDelete="updateChildFilter({}, { parentId: row.parent_id })"
                                        >删除
                                    </DeleteRecord>
                                    <DeleteRecord v-if="!row.parent_id" :params="{ id: row.category_id }" :requestApi="delCategory" @afterDelete="loadFilter"
                                        >删除</DeleteRecord
                                    >
                                </template>
                            </el-table-column>
