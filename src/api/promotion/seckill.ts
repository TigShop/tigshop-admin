...
import { SeckillFilterResult, SeckillFormResult ,SeckillFilterParams} from '@/types/promotion/seckill';
// 获取秒杀列表
...
export const batchSubmit = (type: string, data: object) => {
    return request({
        url: 'promotion/seckill/batch/?type=' + type,
        method: 'post',
        data,
    });
}
...
export const updateSeckillFiled = (data: object) => {
    return request({
        url: 'promotion/seckill/update_field/',
        method: 'post',
        data,
    });
}
...
export const getSeckill = (action: string, params: object) => {
    return request<SeckillFormResult>({
